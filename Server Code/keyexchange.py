    ...
    try:
        ...
        while True:
            ...
            if request[0] is codes.MC_INITIATE:
                ...
                flag = genFlag(name)
                if init[3] == 1:
                    msg = aeskey.pad("youtube.com/watch?v=4YEYWrVU0jo".encode("utf-8"))
                else:
...
# flag generation using name for seed
def genFlag(name):
    flag = ""
    seed = 0
    for char in name:
        seed = (~((ord(char) << 28) ^ 0xAAAAAAAA) + seed) % 69225375724
    random.seed(seed)
    for i in range(12):
        flag = flag + str(random.randint(0, 9))
    return "flag{k3y_eXch&ng3_#" + flag + "}"
